...
from qwen_vl_chat.qwen_generation_utils import make_context
...
class Qwen_VL_Chat(LargeMultimodalModel):
    ...
    def __init__(self, args):
        super().__init__(device=getattr(args, 'device', 'cuda'))
        self.args = args
        self.model = AutoModelForCausalLM.from_pretrained(args.model_path, device_map=getattr(args, "device", "cuda"), trust_remote_code=True)
    ...
    def risk_inputs(self, image_path, prompt):
        ...
        _, ids = make_context(self.tokenizer, query, history=None,
                              system='You are a helpful assistant.',
                              max_window_size=cfg.max_window_size, chat_format=cfg.chat_format)
        return self.model, torch.tensor([ids], device=self.model.device), None

    @torch.no_grad()
    def generate(self, image_path, prompt):
        _, input_ids, _ = self.risk_inputs(image_path, prompt)
        cfg = self.model.generation_config
        if cfg.chat_format != 'chatml':
            raise ValueError('Qwen-VL-Chat generation requires chatml')
        eos = cfg.eos_token_id
        stop_ids = set(eos if isinstance(eos, list) else [eos]) - {None}
        stop_ids.update([self.tokenizer.im_end_id, self.tokenizer.im_start_id])
        kwargs = dict(do_sample=self.args.temperature > 0, num_beams=self.args.num_beams,
                      max_new_tokens=self.args.max_length, use_cache=True,
                      eos_token_id=sorted(stop_ids), return_dict_in_generate=True)
        ...
        if not torch.equal(sequence[:input_ids.shape[1]], input_ids[0]):
            raise RuntimeError('Qwen generation did not preserve input token prefix')
        ids = sequence[input_ids.shape[1]:].tolist()
        return generation_output(self.tokenizer, ids, stop_ids)
    ...
    def _basic_forward(self, image_path, prompt, answer=None, return_dict=False):
        ...
        if answer is not None:
            ...
            context_tokens += [self.tokenizer.im_end_id]
