...
def combine_coco_instances(annotation_path):
    ...
    with open('%s/instances_%s2014.json' %(annotation_path, 'val')) as handle:
        val_instances = json.load(handle)
    with open('%s/instances_%s2014.json' %(annotation_path, 'train')) as handle:
...
class CHAIR(object):
    ...
    def caption_to_words(self, caption):
        ...
        idxs = [idx for idx, word in enumerate(words) \
...
def load_eval_chair(cache, coco_path):
    """Versioned evaluation-only cache; never accept a TokenCHAIR subclass."""
    import fcntl
    from pathlib import Path
    annotation_path = Path(coco_path).expanduser().resolve()
    signature = dict(version='eval_chair_coco2014_v1', annotation_path=str(annotation_path),
                     files={name: [p.stat().st_size, p.stat().st_mtime_ns]
                            for name in ('instances_train2014.json', 'instances_val2014.json',
                                         'captions_train2014.json', 'captions_val2014.json')
                            for p in [annotation_path / name]})
    if not cache:
        evaluator = CHAIR(str(annotation_path))
        evaluator.eval_signature = signature
        return evaluator
    cache = Path(cache).expanduser()
    cache.parent.mkdir(parents=True, exist_ok=True)
    with Path(str(cache) + '.lock').open('a') as lock:
        fcntl.flock(lock, fcntl.LOCK_EX)
        if cache.exists():
            with cache.open('rb') as handle:
                evaluator = pickle.load(handle)
            if type(evaluator) is CHAIR and getattr(evaluator, 'eval_signature', None) == signature:
                print(f'Loaded evaluation CHAIR cache: {cache}', flush=True)
                return evaluator
        print(f'Building evaluation CHAIR with COCO 2014: {cache}', flush=True)
        evaluator = CHAIR(str(annotation_path))
        evaluator.eval_signature = signature
        temporary = Path(str(cache) + '.tmp')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--coco_path", type=str, default='/root/autodl-tmp/dataset/coco/annotations',
                        help="COCO 2014 annotations directory; also checked against cache metadata.")
    ...
    # Canonical module identity also makes newly written pickles reusable by scripts.
    from eval_chair import load_eval_chair
    evaluator = load_eval_chair(args.cache, args.coco_path)

    cap_dict = evaluator.compute_chair(args.cap_file, args.image_id_key, args.caption_key)
    cap_dict['evaluation'] = evaluator.eval_signature
